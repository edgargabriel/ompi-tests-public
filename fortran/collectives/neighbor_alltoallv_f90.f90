...
!!
      ...
      program neighbor_alltoallv_f90
      use mpi
      implicit none
      ...
      if (cart .ne. MPI_COMM_NULL) then
         ...
         if (myself == 0) then
            write(*,*) 'Testing MPI_Neighbor_alltoallv on cartesian communicator'
         end if
      ...
      if (myself == 0) then
         write(*,*) 'Testing MPI_Neighbor_alltoallv on graph communicator'
      end if
      ...
      if (myself == 0) then
         write(*,*) 'Testing MPI_Neighbor_alltoallv on dist graph communicator'
      end if
      ...
      function run_test(expected, edges, comm, out, in) result(res)
        ...
        do while(j <= MAXLEN)
          ...
          call MPI_Neighbor_alltoallv(out, counts, disps, MPI_INTEGER, &
                                      in, counts, disps, MPI_INTEGER, comm, ierr)
      ...
      end program neighbor_alltoallv_f90
