...
program reduce_scatter_test
   ...
   ! Allocate memory
   allocate(in(MAXLEN * tasks), out(MAXLEN * tasks * tasks), recvcounts(tasks), stat=ierr)
   if (ierr /= 0) then
   ...
   do while (j <= MAXLEN * tasks)
      ...
      call MPI_Reduce_scatter(out, in, recvcounts, MPI_INTEGER, MPI_SUM, MPI_COMM_WORLD, ierr)
   ...
   deallocate(in, out, recvcounts)

end program reduce_scatter_test
