...
!!
      ...
      program neighbor_alltoallv_init_f08
      use mpi_f08
      implicit none
      ...
      if (cart .ne. MPI_COMM_NULL) then
         ...
         if (myself == 0) then
            write(*,*) 'Testing MPI_Neighbor_alltoallv_init on cartesian communicator'
         end if

         ! Check cartesian all to all
         rc = run_test(expected, 4, cart, out, in)
         if (rc .ne. 0) then
            write(*,*) 'Neighborhood alltoallv test on cart failed'
            err = err + 1
      ...
      if (myself == 0) then
         write(*,*) 'Testing MPI_Neighbor_alltoallv_init on graph communicator'
      end if

      rc = run_test(expected, num_edges, graph, out, in)
      if (rc .ne. 0) then
         write(*,*) 'Neighborhood alltoallv test on graph failed'
         err = err + 1
      else if (myself == 0) then
         write(*,*) 'Pass!'
      end if
      ...
      call MPI_Dist_graph_create_adjacent(MPI_COMM_WORLD, num_edges, expected, MPI_UNWEIGHTED, &
                                         ...
                                         .false., dist_graph)
      if (myself == 0) then
         write(*,*) 'Testing MPI_Neighbor_alltoallv_init on dist graph communicator'
      end if

      rc = run_test(expected, num_edges, dist_graph, out, in)
      if (rc .ne. 0) then
         write(*,*) 'Neighborhood alltoallv test on dist graph failed'
         err = err + 1
      else if (myself == 0) then
         write(*,*) 'Pass!'
      end if
      ...
      function run_test(expected, edges, comm, out, in) result(res)
        integer, intent(in) :: edges
        integer, intent(in), dimension(0:edges-1) :: expected
        type(MPI_Comm), intent(in) :: comm
        integer, intent(inout), dimension(0:) :: out, in
        integer :: res
        integer, dimension(0:edges-1) :: counts, disps
        integer :: i, j, k
        ...
        do while (j <= MAXLEN)
          ...
          call MPI_Neighbor_alltoallv_init(out, counts, disps, MPI_INTEGER, &
                                           in, counts, disps, MPI_INTEGER, comm, MPI_INFO_NULL, req)
          call MPI_Start(req)
          call MPI_Wait(req, MPI_STATUS_IGNORE)
          call MPI_Request_free(req)
      ...
      end program neighbor_alltoallv_init_f08
