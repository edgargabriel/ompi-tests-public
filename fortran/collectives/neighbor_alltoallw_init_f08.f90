...
!!
      ...
      program neighbor_alltoallw_init_f08
      use mpi_f08
      implicit none
      ...
      if (cart .ne. MPI_COMM_NULL) then
         ...
         if (myself == 0) then
            write(*,*) 'Testing MPI_Neighbor_alltoallw_init on cartesian communicator'
         end if

         ! Check cartesian all to all
         rc = run_test(expected, 4, cart, out, in, datatypes_out, datatypes_in)
         if (rc .ne. 0) then
            write(*,*) 'Neighborhood alltoallw_init test on cart failed'
            err = err + 1
      ...
      if (myself == 0) then
         write(*,*) 'Testing MPI_Neighbor_alltoallw_init on graph communicator'
      end if

      rc = run_test(expected, num_edges, graph, out, in, datatypes_out, datatypes_in)
      if (rc .ne. 0) then
         write(*,*) 'Neighborhood alltoallw_init test on graph failed'
         err = err + 1
      else if (myself == 0) then
         write(*,*) 'Pass!'
      end if
      ...
      call MPI_Dist_graph_create_adjacent(MPI_COMM_WORLD, num_edges, expected, MPI_UNWEIGHTED, &
                                         ...
                                         .false., dist_graph)
      if (myself == 0) then
         write(*,*) 'Testing MPI_Neighbor_alltoallw_init on dist graph communicator'
      end if

      rc = run_test(expected, num_edges, dist_graph, out, in, datatypes_out, datatypes_in)
      if (rc .ne. 0) then
         write(*,*) 'Neighborhood alltoallw_init test on dist graph failed'
         err = err + 1
      else if (myself == 0) then
         write(*,*) 'Pass!'
      end if
      ...
      function run_test(expected, edges, comm, out, in, datatypes_out, datatypes_in) result(res)
        ...
        do while (j <= MAXLEN)
          ...
          call MPI_Neighbor_alltoallw_init(out, counts, disps_bytes, datatypes_out, &
                                           in, counts, disps_bytes, datatypes_in, comm, MPI_INFO_NULL, req)
          call MPI_Start(req)
      ...
      end program neighbor_alltoallw_init_f08
