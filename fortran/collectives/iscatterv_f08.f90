...
program iscatterv_test
   use mpi_f08
   implicit none

   integer, parameter :: MAXLEN = 10000
   integer, allocatable :: out(:), in(:), displs(:), rcounts(:)
   integer :: root, i, j, k
   ...
   ! Allocate memory
   allocate(in(MAXLEN), out(MAXLEN * tasks), displs(tasks), rcounts(tasks), stat=ierr)
   if (ierr /= 0) then
   ...
   do while (j <= MAXLEN)
      ...
      call MPI_Iscatterv(out, rcounts, displs, MPI_INTEGER, in, j, MPI_INTEGER, &
                         root, MPI_COMM_WORLD, request, ierr)
